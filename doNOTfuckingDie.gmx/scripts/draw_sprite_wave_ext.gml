...
//      color       color _blending, real
//      alpha       alpha _blending, real
//
/// GMLscripts.com/license
{
    var sprite,image,posx,posy,axis,wavelength,amplitude,phase,xscale,yscale,_blend,alpha;
    sprite = argument0;
    image = argument1;
    posx = argument2;
    posy = argument3;
    ...
    for (i=0; i<size; i+=1) {
        ...
        if (axis) {
            ...
            draw_sprite_part_ext(sprite,image,0,i,width,1,sx,sy,xscale,yscale,_blend,alpha);
        }else{
            sx = xscale*(i-xoff)+posx;
            sy = yscale*(shift-yoff)+posy;
            draw_sprite_part_ext(sprite,image,i,0,1,height,sx,sy,xscale,yscale,_blend,alpha);
        }
    }
    return 0;
}
