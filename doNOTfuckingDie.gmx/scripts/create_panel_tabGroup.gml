/// create_panel_tabGroup(GROUP_name,rel_x,rel_y,width,height,LABEL_value,[tab1_name],[tab1_value],[tab1_active],[tab2_name],[tab2_value],[tab2_active],[tab3_name],[tab3_value],[tab3_active])
...
{
    var name,rel_x,rel_y,width,height,value,tab_count;
    name = argument[0];
    rel_x = argument[1];
    rel_y = argument[2];
    width = argument[3];
    ...
    for(var i = 6; i < argument_count; i+=3)
    {
        ...
        var yy = y;
        // create tabs
        with(instance_create(xx, yy, obj_ui_radio2))
        {
            // change dimensions on both component and fl
            set_sprite_size(tab_width,tab_height /*- tab_offset*/); 
            // build fl
            f = fl_add_radio(tab_name, 0, 0, sprite_width, sprite_height, tab_value, name, tab_active);
        }
    }
}
