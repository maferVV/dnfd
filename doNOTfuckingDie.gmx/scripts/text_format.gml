/// text_format(text, [font, w])

var text=argument[0], font, maxw, a=array_create(0), q=ds_queue_create(), mw=0, mh=0, W=0, H=0, a_n=1;
...
for(var i=1; i<=string_length(text); i++)
{
    if string_copy(text, i, 6)='<<tag#'
    {
        ...
        if tag=ts.image
        {
            ...
            if h=ts.standart h=string_height('A')
            if w=ts.standart
            {
    ...
    else
    {
            ...
            while mw+wordw>=maxw and !ex and maxw!=-1
            {
                ...
                for(var z=1; z<=string_length(word)-3; z++)
                {
                    ...
                    if mw+nsize+string_width(symb)>=maxw or z=string_length(word)-3
                    {
                        ...
                        wordw=string_width(word)
                        break
                    }
                    nword+=symb
                    nsize+=string_width(symb)
                }
                if ex break
            }
        ...
        if a_n!=1 or word!=' '
        {
            if global.ts_j=1
            {
                ...
                a[a_n-1]+=word
            mw+=string_width(word)
        }
        mh=max(mh, string_height('A'))
        W=max(W, mw)
    }
}
