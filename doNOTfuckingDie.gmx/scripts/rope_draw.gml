...
_h_half = rp_height[/*@ */_id]/2            *objCamera.camera_pitch; // modify height based on pitch, rope segment anglo towadrs camera
...
for (i=0; i<_cuts-1; i++) {
  ...
  for (j=a; j<b/*+1*/; j++) 
  {
    ...
    if debug draw_debug_point(_x1, _y1, string(j)+".a", 2);
    draw_set(1, c_white, fnt_mono, fa_right);
    if debug draw_debug_point(_x2, _y2, string(j)+".b", 2);
    draw_set();
